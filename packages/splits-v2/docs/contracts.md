...
Splits V2 is the core splitting protocol from 0xSplits. It provides onchain payment splitting through an architecture
...
2. **Warehouse**: For pull splits, the wallet deposits tokens into the SplitsWarehouse, which tracks balances as ERC6909
   token positions. Token IDs are `uint256(uint160(tokenAddress))`.
3. **Distribute**: Anyone can call `distribute()` on a split wallet, which calculates each recipient's share based on
   their allocation and sends funds accordingly.
...
Funds are deposited into the SplitsWarehouse and credited to each recipient as ERC6909 balances. Recipients call
`withdraw()` on the warehouse to claim their tokens.
...
`ERC1271` (for signature validation).
...
Key functions: `getHash()`, `validate()`, `getDistributions()`, `calculateAllocatedAmount()`,
`calculateDistributorReward()`.
